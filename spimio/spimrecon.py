...
def slab_recon(path, level=0):
    ...
    for chunk in info['MetaChunks']:
        ...
        slicer = [slice(math.ceil(o), math.ceil(o)+s)
                  for o, s in zip(off, d.shape)]
        view = out[(idx, *slicer)]

        grid = [np.arange(s, dtype='float32').__iadd__(o-math.ceil(o))
                for o, s in zip(off, d.shape)]
...
def all_slabs_recon(path, level=0, proj=None):
    ...
    min_index = min(slab_indices)
    max_index = max(slab_indices)
    ...
    for idx, slab in zip(slab_indices, slabs):
        idx = idx - min_index
